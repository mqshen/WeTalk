...
object WeTalkLoadDriver {
  ...
  val initialMessagesPerSecond = clientConfig.getInt("initial-messages-sent-per-second")
  ...
  val connect = addresses map (
    _ split (":") match {
      case Array(host: String, port: String) => new InetSocketAddress(host, port.toInt)
    })
...
class WeTalkLoadDriver extends Actor with ActorLogging {
  ...
  private var nMessagesSentPerSecond = initialMessagesPerSecond
  ...
  def receive = {
    case RoundBegin(nConns) =>
      ...
      nConnections = nConns
      println("---------------- Concurrent connections " + nConnections + {
        if (isBroadcast) " (broadcast)" else " (single bounce)"
      } + " ----------------")
  ...
  private def performLoadTest() {
    ...
    nMessagesSentPerSecond = initialMessagesPerSecond
  ...
  private def triggerMessages(statistics: mutable.Map[Double, StatisticalSummary], _overallEffectiveRate: Double) {
    ...
    isMessagesSent = true
    println(nMessagesSent + " msgs " + { if (isBroadcast) "broadcast" else "sent" } + " in "
      + (System.currentTimeMillis - t0) + "ms, expect to receive " + nMessagesExpected)
