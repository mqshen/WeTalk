...
trait ConnectionActor extends Actor with ActorLogging with FSM[Connection.ConnectionState, Connection.ConnectionData] {
  ...
  protected def userActor: ActorRef
...
class Connection(_remoteAddress: InetSocketAddress, databaseActor: ActorRef) extends ConnectionActor {

  override protected val remoteAddress = _remoteAddress

  override protected val userActor = context.actorOf(UserActor.props(self, databaseActor), "user")
...
object Connection {
  ...
  def props(remoteAddress: InetSocketAddress, databaseActor: ActorRef) = {
    Props(classOf[Connection], remoteAddress, databaseActor)
  }
